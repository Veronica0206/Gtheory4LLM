...
if (smoke && !length(args)) stop("A smoke run requires an explicit empty output directory.")
...
file.copy(file.path(study_dir, "run.R"), file.path(output_dir, "run-executed.R"))
config <- read.csv(file.path(study_dir, "config.csv"), stringsAsFactors = FALSE)
if (smoke) config$replicates <- 1L
...
relative_files <- c("DESCRIPTION", "load_functions.R",
  ...
  "validation-studies/gaussian-coverage/summarize.R")
...
save_csv(data.frame(package = c("Gtheory4LLM (checkout)", "OpenMx", "R"),
  version = c(read.dcf(file.path(root, "DESCRIPTION"))[1, "Version"],
              as.character(utils::packageVersion("OpenMx")), as.character(getRversion()))),
  "environment.csv")
...
source(file.path(study_dir, "summarize.R"), local = TRUE)
