...
| `identity` | — | `1e-10` | `laplace_identity_residual` and `hessian_asymmetry` are algebraic identities, not stationarity residuals, and must not borrow the `1e-6` mode-gradient allowance. They reconstruct exactly up to rounding in the same sums the `objective` tolerance already covers, so they are held at that same absolute rounding scale. Deliberately not tightened further after the fact: the observed values sit near `1e-15`, and a threshold moved to fit what was measured is not a predeclared tolerance. |
