...
Publishing, tagging, and submitting to CRAN are maintainer actions and are not performed by any workflow here. Until a version tag exists, the bundle in `artifacts/` is `prepared`, not `published`, and nothing in the repository may describe it otherwise; the release-identity check enforces this in both directions. See [the release checklist](RELEASE_CHECKLIST.md) for the sequence and [the repository policy](REPOSITORY_POLICY.md) for required checks, protection settings, and the committed-binary decision.
